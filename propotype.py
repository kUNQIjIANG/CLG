...
batch_sent = tf.placeholder(tf.int32, shape = [None, max_len])
input_sent = tf.placeholder(tf.int32, shape = [None, max_len])
enc_seq_len = tf.placeholder(tf.int32, shape = [None])
...
encoder_output, encoder_state = tf.nn.dynamic_rnn(encoder_cell, sour_embed,dtype = tf.float32,
                                                  #initial_state = shit_state,
...
attention_mechanism = tf.contrib.seq2seq.LuongAttention(hid_units,
                                                        attention_states,
                                                        memory_sequence_length = enc_seq_len) # len of attention_states = source_seq_len
...
seq_mask = tf.cast(tf.sequence_mask([3,5,6],max_len),tf.float32)
...
with tf.Session() as sess:
    ...
    outputs, ind, s_l, k_l, los, _ = sess.run([logits,hightest_indice,seq_loss,kl_loss,loss, optimizer],
                                                feed_dict = {batch_sent : test_sent,
                                                             input_sent : enc_id_matrix,
