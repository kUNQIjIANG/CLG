...
class Discriminator(botModel):
	def __init__(self,c_size, hid_units, batch_size, word_embed, embed_size):
	...
	def build_graph(self):
		with tf.variable_scope(self.scope):
			...
			enc_output, enc_final_state = tf.nn.dynamic_rnn(self.encoder_cell, self.enc_inp, dtype = tf.float32, sequence_length = self.enc_len)
